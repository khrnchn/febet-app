...
import { formatTimeRange, formatTime } from './utils/format'
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)]">
      <LoadingModal isOpen={isOptimizing} />
      <div className="flex flex-1 h-full">
        ...
        <div className="flex-none w-[400px] border-r overflow-hidden flex flex-col">
          ...
          <div className="border-b">
            <div className="flex">
              <button
                className={`flex-1 px-4 py-3 text-sm font-medium ${activeTab === 'deliveries'
              ...
              <button
                className={`flex-1 px-4 py-3 text-sm font-medium ${activeTab === 'orders'
                  ? 'border-b-2 border-primary text-blue-600'
                  : 'text-gray-500 hover:text-gray-700'
                  }`}
                ...
                <div className="p-4 border-b">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="lg"
                      className="w-full uppercase h-12 font-bold tracking-wide"
                    >
                      <svg
                      ...
                      {isOptimizing ? "Generating deliveries..." : "Generate Deliveries"}
                    </Button>
                  </div>
                  <p className="pt-1 text-gray-500 text-sm">Use AI to generate the most optimized bulk deliveries</p> 
                </div>
                {
                  directions ? (
                    <RouteInfo
                    ...
                    <div className="p-4 text-sm text-gray-500">
                      No deliveries yet. Click "Generate Deliveries" to start.
                    </div>
                  )
                }
              </>
                  ...
                  {localOrders.map((order) => (
                    <div
                      ...
                      className={`p-4 rounded-lg cursor-pointer transition-colors group ${
                        selectedDelivery?.id === order.id
                          ...
                          : 'bg-muted hover:bg-muted/80 text-gray-600'
                      }`}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3">
                        ...
                        <span className="text-sm col-span-2">{order.productId}</span>
                      </div>
                      <div className="grid grid-cols-3">
                        <span className="text-sm">Customer</span>
                        <span className="text-sm col-span-2">{order.customerName}</span>
                      </div>
                      <div className="grid grid-cols-3">
                        <span className="text-sm">Contact</span>
                        <span className="text-sm col-span-2">{order.customerContact}</span>
                      </div>
                      <div className="grid grid-cols-3">
                        <span className="text-sm">Address</span>
                        <span className="text-sm col-span-2">{order.destination}</span>
                      </div>
                      <div className="grid grid-cols-3">
                        <span className="text-sm">Time</span>
                        <span className="text-sm col-span-2"> {formatTimeRange(order.deliveryStart, order.deliveryEnd)}</span>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  return (
    <div className="flex flex-col gap-4 bg-muted">
      {batchInfo && (
        <div className="p-4">
          <div className="mb-4">
            <h3 className="text-lg font-semibold">List of Delivery Batches</h3>
            <div className="gap-1 text-sm grid grid-cols-3 text-center text-gray-800 pt-1">
              <span>Batches: <br/>{batchInfo.totalBatches}</span>
              <span>Deliveries: <br/>{batchInfo.totalDeliveries}</span>
              <span>Deliveries/Batch: <br/>{batchInfo.averageDeliveriesPerBatch.toFixed(1)}</span>
            </div>
          </div>
          ...
          {/* Total Price Summary */}
            <div className="flex flex-col gap-2 pb-4">
              <Button
                size="lg"
                variant="outline"
                className="w-full uppercase h-12"
                onClick={() => {
          ...
          <div className="space-y-4">
            {(batchInfo.batches || []).map((batch, batchIndex) => {
              ...
              return (
                <div
                  key={batch.id}
                  className={`p-4 bg-white rounded-md cursor-pointer transition-colors border border-2 ${selectedBatchId === batch.id
                    ? 'border-primary'
                    : 'hover:bg-muted/80 drop-shadow-md border-white'
                    }`}
                ...
                >
                  ...
                  {/* Summary Stats */}
                  <div className="gap-1 text-sm grid grid-cols-3 text-center text-gray-700 mb-3">
                        ...
                        <div key={index} className="pl-4 border-l-2 border-muted-foreground/20">
                          ...
                          <div className="flex gap-3 mt-2">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs opacity-90">
                                ETA: {delivery.estimatedArrival}pm
                              </span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <svg className="w-3.5 h-3.5 opacity-70" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
